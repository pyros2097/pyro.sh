...
const Bible = ({ item }) => {
  ...
  const onVerseClick = (i) => {
    ...
    setHighlightedVerses(index === -1 ? higlightedVerses.concat([i]) : higlightedVerses.filter((item) => item !== i));
  };
                  ...
                  >
                    <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                      ${bible[book].map((item, index) => {
                        return html`
                          <button
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 ${chapter === index + 1 ? 'bg-gray-100' : ''}"
...
defineElement('app-bible', Bible, attrTypes);
