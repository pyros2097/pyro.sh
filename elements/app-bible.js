...
const AppBible = ({ title }) => {
  ...
  const fetchBible = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/assets/bibles/${title.toLowerCase()}.json`);
    ...
    if (playing) {
      ...
      for (const i of higlightedVerses) {
        await new Promise((resolve) => {
          const audio = new Audio(`/assets/audio/kannada/${book}/chapter_${chapter}/verse_${i + 1}.mp3`);
          audio.loop = false;
          audio.play();
      ...
      <div class="w-1/6 sm:w-auto flex flex-1 flex-col p-0 ml-0 sm:px-4 sm:ml-2">
        <div class="flex flex-row items-center border-b border-bibleborder py-2 px-2">
          <div class="flex flex-row items-center">
            <button class="sm:hidden mr-2" @click=${() => setShowChapters(true)}>
...
AppBible.observedAttributes = ['title'];

export default AppBible;
